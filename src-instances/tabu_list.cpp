...
full_tabu_list::full_tabu_list(int length, int m, int k, bool aspire) {
...
void
full_tabu_list::add(int i, int j, double value) {
...
void
full_tabu_list::add_old_and_remove_new( int i, int old_j, double old_distance, int new_j ) {
...
bool
full_tabu_list::is_tabu(const int i, const int j) const {
...
bool
full_tabu_list::does_aspire(const int i, const int j, double value) const {
...
bool
full_tabu_list::can_be_reassigned(const int i, const vector<double> distances, int current_j ) {
...
void
full_tabu_list::remove_tabu_pair(const int i, const int j) {
  table[i][j] = -1;		//! the pair is no more tabu
  tabu_value[i][j] = -1;
}

void
full_tabu_list::full_print_tabu_list() const {
